...
class FelicitaClient:
    ...
    async def async_connect(self) -> None:
        ...
        try:
            ...
            raise ConfigEntryNotReady(f"Failed to connect to device: {error}")
    ...
    def _notification_callback(self, _: int, data: bytearray) -> None:
        ...
        # Parse battery - byte 15 contains battery level
        battery_raw = data[15]
        self._battery = round(((battery_raw - MIN_BATTERY_LEVEL) / 
                             (MAX_BATTERY_LEVEL - MIN_BATTERY_LEVEL)) * 100)

        # Parse unit - bytes 9-11 contain unit
        self._unit = data[9:11].decode('utf-8').strip()
